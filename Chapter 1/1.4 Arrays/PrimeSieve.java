...
public class PrimeSieve {
    public static void main(String[] args) {
        ...
        //Initialy set all integers as prime
        boolean[] isPrime = new boolean[n + 1];
        for (int i = 2; i <= n; i++) {
            isPrime[i] = true;
        }

        // Use the Sieve of Eratosthenes to mark all the non prime numbers as n
        for (int factor = 2; factor <= n; factor++) {

            if (isPrime[factor]) {
                // if the factor is prime make every multiple of it false
        ...
        // Count up all of the prime numbers that you got using a for loop
        int primeNums = 0;
        for (int t = 2; t < n; t++) {
        ...
        //Display how many prime numbers are contained into the part
        System.out.println(n + " has " + primeNums + " Prime numbers.");
